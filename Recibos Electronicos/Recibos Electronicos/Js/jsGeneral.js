...
function VerReporteExportarConceptos(Opc, UR, FInicial, FFinal, Conceptos, Ejercicio, Mes, ciclo, TipoDesc, Status, MesFin, enExcel, Nivel, conGraficas) {
    window.open('Reportes/VisualizadorCrystal.aspx?Tipo=' + Opc + '&CDet=' + UR + '&FInicial=' + FInicial + '&FFinal=' + FFinal + '&IdConcepto=' + Conceptos + '&Ejercicio=' + Ejercicio + '&Mes=' + Mes + '&ciclo=' + ciclo + '&TipoDesc=' + TipoDesc + '&Status=' + Status + '&MesFin=' + MesFin + '&enExcel=' + enExcel + '&Nivel=' + Nivel + '&conGraficas=' + conGraficas, 'miniContenedor', 'toolbar=yes', 'location=no', 'menubar=no');
  return false;
}

function OpenPage(url) {
    //window.open(url, 'Contenedor','width=100%,height=100%')
    ; //return false;
