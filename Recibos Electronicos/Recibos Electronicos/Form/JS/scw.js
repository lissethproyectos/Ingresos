...
//******************************************************************************
    ...
    function scwSetDefaultLanguage()
        {try
            ...
            {// English
             scwToday               = 'Hoy:';
             scwClear               = 'Limpiar';
             scwDrag                = 'click para mover ventana';
             scwArrMonthNames       = ['Ene','Feb','Mar','Abr','May','Jun',
                                       'Jul','Ago','Sep','Oct','Nov','Dic'];
             scwArrWeekInits        = ['D','L','M','X','J','V','S'];
             scwInvalidDateMsg      = 'La fecha introducida no es válida.\n';
             scwOutOfRangeMsg       = 'La fecha introducida esta fuera del rango.';
             scwDoesNotExistMsg     = 'La fecha introducida no existe.';
             scwInvalidAlert        = ['Fecha inválida (',') ignorada.'];
             scwDateDisablingError  = ['Error ',' no es un objeto de fecha.'];
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            '/* Calendar background */'                                 +
            'table.scw                  {background-color:  #77B0D2;}'  +
            '/* Drag Handle */'                                         +
            'td.scwDrag                 {background-color:  #3D78B4;'   +
                                        'color:             #CCCCFF;}'  +
            '/* Week number heading */'                                 +
            'td.scwWeekNumberHead       {color:             #0f8fff;}'  +
            '/* Week day headings */'                                   +
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwNow                  {background-color:  #282aff;'   +
                                        'color:             #FFFFFF;}'  +
            '/* Clear Button */'                                        +
...
///funciones de verificacion de fecha
function esFechaValida(fecha){
    if (fecha != undefined && fecha.value != "" ){
        if (!/^\d{2}\/\d{2}\/\d{4}$/.test(fecha.value)){
            alert("La fecha debe contener el siguiente formato dd/mm/aaaa " + fecha.value);
            return false;
        }
    ...
    switch(mes){
        ...
        case 2:
            ...
            break;
        default:
            alert("Fecha introducida errónea, verifique que contenga el siguiente formato dd/mm/aaaa");
            return false;
    }
